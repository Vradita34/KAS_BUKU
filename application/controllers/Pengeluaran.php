...
class Pengeluaran extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->db->where('jenis_transaksi', 'Pengeluaran'); // Filter jenis transaksi
		$this->db->order_by('date', 'ASC');
		$ajg = $this->db->get()->result_array();
		$data = array('pengeluaran' => $ajg);
		$this->template->load('layout/template', 'pengeluaran', $data);
	}
	public function tambah()
	{
		...
		$this->template->load('layout/template', 'pengeluaran_tambah');
	}
		...
		if ($id_transaksi) {
			...
			<div class="alert alert-success left-icon-big alert-dismissible fade show">
				...
				<div class="media">
					...
					<div class="media-body">
						<h5 class="mt-1 mb-1">Congratulations!</h5>
						<p>Berhasil menambah data Pengeluaran ^_^ !</p>
					</div>
				</div>
			</div>
